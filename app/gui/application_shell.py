...
class ProjectAegisApplicationShell:
    def __init__(
        ...
        *,
        ...
        self.action_approval_view = ActionApprovalView(
            self.frame, action_approval_presenter, on_action_decision
        )
        ...
        self.external_tool_view = ExternalToolView(
            self.frame, tool_budget_presenter
        )
        self.budget_view = BudgetView(self.frame, tool_budget_presenter)
        for view in (
            self.input_view,
            self.progress_view,
            self.result_panel,
    ...
    def render_execution_results(
        self, results: tuple[ExecutionAnalysisResult, ...]
    ) -> None:
    ...
    def _on_event_state_changed(self, state: ApplicationState) -> None:
        self.render(state)
